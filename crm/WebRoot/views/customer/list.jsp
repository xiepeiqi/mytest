...
<form method="post" action="<s:url value="/customer_list"/>">
	<div class="text-c"> 
		<s:select name="query.custSource" list="{'朋友介绍','已购买产品客户','来自网络搜索'}" headerKey="" headerValue="==全部信息来源=="></s:select>
		...
		<select name="query.custLevel" id="query_custLevel">
		    ...
		    <option value="1" <s:if test="#session.query.custLevel == 1">selected="selected"</s:if>>普通客户</option>
		    <option value="2" <s:if test="#session.query.custLevel == 2">selected="selected"</s:if>>金牌客户</option>
		    <option value="3" <s:if test="#session.query.custLevel == 3">selected="selected"</s:if>>白金客户</option>
		</select>
	...
	<table class="table table-border table-bordered table-hover table-bg">
		...
		<tbody>
			<s:iterator value="#pb.curPageData" status="sta">
			<tr class="text-c">
				<td><input type="checkbox" value='<s:property value="custId"/>' name="custIDS"></td>
				<td><b><s:property value="((#pb.curPage-1)*#pb.pageSize)+(#sta.index+1)"/></b></td>
				<td><s:property value="custName"/></td>
				<td><s:property value="custSource"/></td>
				<td><s:property value="custIndustry"/></td>
				<td>
					<s:if test="custLevel==1">普通客户</s:if>
					<s:if test="custLevel==2">金牌客户</s:if>
					<s:if test="custLevel==3">白金客户</s:if>
